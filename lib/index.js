...
const schema = Joi.object({
  ...
  role: Joi.string().when('local', { is: true, then: Joi.optional(), otherwise: Joi.required() }).description('AWS role with execute permissions'),
  deploy: Joi.object().keys({
    ...
    memory: Joi.number().positive().integer().multiple(64).optional().default(Bundler.defaults.lambda.memory).description('allocated function memory in MB'),
    timeout: Joi.number().positive().integer().optional().default(Bundler.defaults.lambda.timeout).description('function execution timeout in seconds'),
